...
最終行番号を取得する（これが追記開始行の基準になる）。
B列（ドキュメント名）の全登録済みタイトル一覧を取得する。これを「既存リスト」として重複チェックに使う。


Step 2: Hubble から未登録ドキュメントを検出

https://admin.hubble-docs.com/admin/folders/organization にアクセスする。

10ティック × 8回スクロールダウンして、全ドキュメントのタイトルを取得する。
...
https://www.cloudsign.jp/team/documents?req.Page=1&req.Type.Ongoing=true&req.Type.Completed=true&req.Type.Canceled=true&req.Type.Imported=true にアクセスする。

10ティック × 8回スクロールダウンして、全ドキュメントのタイトルを取得する。

対象行の契約書名（C列）またはドキュメント名（B列）から抽出した相手方名と一致するものを照合する。

照合できたドキュメントの From: の後ろにある氏名（文字情報）またはメールアドレスを取得する。

照合できない場合は、担当者を空欄要確認とする（既存の値を上書きしない）。


Step 5: name-lisets で担当者名・Section を解決する

https://docs.google.com/spreadsheets/d/1k7WABCI8pBxODDPqVu-ktlFi1SGEgw8KMR2KXyfvK8U/edit?gid=311919137#gid=311919137 にアクセスする（シート名: name-lisets）。
                                  ...
                                  Logger.log('追記対象なし');
                                      return;
                                          ...
                                          meibo.getRange(lastRow + 1, 1, toAppend.length, toAppend[0].length).setValues(toAppend);
                                            ...
                                            担当者（N列）: 第1優先はCloudSign照合 → name-lisets C列で氏名解決。第2優先は契約書本文。取得不能時は空欄要確認。
                                            Section（M列）: 第1優先はname-lisets B列（担当者名またはメールで検索）。第2優先は契約書本文。取得不能時は空欄要確認。
                                            契約形態（O列）: 第1優先はHubbleドキュメント名キーワード判定。取得不能時は空欄要確認。


                                            エラーハンドリング

                                            get_page_text で No semantic content element found が返った場合は screenshot で代替する。
                                            CloudSign で照合できない場合は担当者を空欄要確認でスキップする（既存値は上書きしない）。
                                            name-lisets で担当者名が見つからない場合は Section を空欄要確認とする。
                                            フィールド抽出不能の場合は空欄のまま暫定登録要確認フラグを付与する。
                                            重複チェックはB列（ドキュメント名）を主キーとして照合し、重複があればスキップしてログに記録する。
                                            処理完了後にサマリーレポートを出力する（追記件数 / スキップ件数 / 要確認件数）。
